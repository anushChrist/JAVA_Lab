...
/*public class GameLogic {
    ...
    public static GameState checkForCompletion(int[][] grid) {
        if (sudokuIsInvalid(grid))
            return GameState.ACTIVE; // if game s
        if (tilesAreNotFilled(grid))
            return GameState.ACTIVE; // if all tiles not filled then game still active
     ...
     * switch (value) {
     * case TOP:
     * if (squareIsInvalid(0, 0, grid)) return true; // left square
     * if (squareIsInvalid(0, 3, grid)) return true; // middle square
     * if (squareIsInvalid(0, 6, grid)) return true; // right sqaure
     * case MIDDLE:
     ...
     * 
     * while (yIndex < yIndexEnd) { // traversing the square so 2D -> 1D array
     * while (xIndex < xIndexEnd) {
     * square.add(grid[xIndex][yIndex]);
     * xIndex++;
     * }
     * xIndex -= 3; // reset back to starting position
     * yIndex++;
     * }
     * 
     * if (collectionHasRepeats(square)) return true; // if a number is repeater
     * then the square is invalid
     * return false;
     * }
    ...
    public static boolean collectionHasRepeats(List<Integer> collection) {
        for (int index = 1; index <= GRID_BOUNDARY; index++) {
            if (Collections.frequency(collection, index) > 1)
                return true; // any number has frequency greater than 1
        }
        return false;
    }
        ...
        for (int xIndex = 0; xIndex < GRID_BOUNDARY; xIndex++) {
            for (int yIndex = 0; yIndex < GRID_BOUNDARY; yIndex++) {
                if (grid[xIndex][yIndex] == 0)
                    return true; // if tile empty
            }
        }
        return false; // all tiles filled
    }
}*/

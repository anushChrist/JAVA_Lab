...
/*public class SudokuSolver {
    ...
    public static boolean puzzleIsSolvable(int[][] puzzle) {
        ...
        Coordinates[] emptyCells = typeWriterEnumerate(puzzle); // 2D array -> 1D array
        ...
        while (index < 40) {
            ...
            while (input < 10) {
                ...
                // everytime we add a number we check if puzzle is still valid
                if (GameLogic.sudokuIsInvalid(puzzle)) { // if invalid
                    if (index == 0 && input == GRID_BOUNDARY) { // reached 9 and at first cell and still violate Sudoku
                                                                // condition -> no solution
                        return false;
                    } else if (input == GRID_BOUNDARY) { // reached 9 then backtrack
                        index--;
                    }
                    input++; // try next input value
                } else { // if valid
                    index++; // go to the next cell
                    if (index == 39)
                        return true; // all cells filled
                    input = 10; // end inner loop
                }
            }
        }
        return false;
    ...
    private static Coordinates[] typeWriterEnumerate(int[][] puzzle) { // typeWriter order / left -> rigth / top ->
                                                                       // bottom
        Coordinates[] emptyCells = new Coordinates[40];
        int iterator = 0;

        for (int y = 0; y < GRID_BOUNDARY; y++) { // top -> bottom
            for (int x = 0; x < GRID_BOUNDARY; x++) { // left -> right
                if (puzzle[x][y] == 0) { // if empty
                    emptyCells[iterator] = new Coordinates(x, y);
                    if (iterator == 39)
                        return emptyCells; // all empty cells found
                    iterator++; // otherwise increment iterator
                }
            }
        }
        return emptyCells;
